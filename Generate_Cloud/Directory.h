    ...
    if (input.graph_dependent_cloud_size)
    {
        ...
        else if (input.pattern_type == "squares")
        {
            directory = input_directory + "Graph_Dependent_Cloud_Size/" + Num_To_String( input.cloud_size_parameter ) +
                "/" + input.noise_type + "/" + Num_To_String( input.noise_parameter ) + "/" + input.pattern_type + "/" + Num_To_String( input.pattern_size_1 ) + "/" + Num_To_String( Betti_num );
        }
        ...
        if (input.pattern_type == "grid")
        {
            if (input.regular)
            {
                directory = input_directory + "/Cloud_" + Num_To_String( input.cloud_size_parameter ) +
                    "/" + input.noise_type + "/" + Num_To_String( input.noise_parameter ) + "/" + input.pattern_type +
                    "/Regular/" + Num_To_String( input.pattern_size_1 ) + "," + Num_To_String( input.pattern_size_2 );
            }
            ...
            else
            {
                directory = input_directory + "/Cloud_" + Num_To_String( input.cloud_size_parameter ) +
                    "/" + input.noise_type + "/" + Num_To_String( input.noise_parameter ) + "/" + input.pattern_type +
                    "/Varied/" + Num_To_String( input.pattern_size_1 ) + "," + Num_To_String( input.pattern_size_2 );
            }
        }
